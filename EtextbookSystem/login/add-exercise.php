<?php
	...
	$queryExercise = $databaseHandle->query('SELECT * FROM exercise');
	$exercise = array();
	while($row = $queryExercise->fetch()){
		$exercise[] = $row;
	}

	if (isset($_POST['submit'])) {
		$queryMaxExerciseId = $databaseHandle->query('SELECT MAX(id) AS max_id FROM exercise FOR UPDATE');
		$maxExerciseId = $queryMaxExerciseId->fetchObject();
		$newExerciseId = $maxExerciseId->max_id + 1;
		$entryDate = date('Y-m-d H:i:s');
		$contentFilename = 'Exercise_Id-' . $newExerciseId;
		$contentLanguage = $_POST['contentLang'];
		$contentDescriptionTag = $_POST['contentTag'];
		$queryAddExercise = 'INSERT INTO exercise (entry_date, content_file_name, content_language, content_description_tag) VALUES (?,?,?,?)';
		$queryHandle = $databaseHandle->prepare($queryAddExercise);
		...
		$exerciseFolderPath = $_SERVER['DOCUMENT_ROOT'] . '/upload/exercise/';
		$exerciseFolder = $contentFilename;
		$filePath = $exerciseFolderPath . $exerciseFolder;
...
<h1 class = "ui center aligned header">Add Exercise Files</h1>
...
<div class="ui section divider"></div>
<?php if(count($exercise)>0) {?>
		<h1 class = "ui center aligned header">Available Exercises</h1>
		<div class = "ui center aligned middle aligned grid">
			...
			<?php foreach($exercise as $e){ ?>
			<div class="five column row">
				<div class="column"><?=$e['id']?></div>
				<div class="column"><?=$e['entry_date']?></div>
				<div class="column"><a href="../upload/exercise/<?=$e['content_file_name']?>" target="_blank"><?=$e['content_file_name']?></a></div>
				<div class="column"><?=$e['content_language']?></div>
				<div class="column"><?=$e['content_description_tag']?></div>
			</div>
			<?php } ?>
		</div>
<?php }?>
